...
// Structure liturgique (Credo supprimé comme demandé précédemment)
...
export default function PlaylistDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleImportOrdinary = async (massName: string) => {
      ...
      setSelectedOrdinary('');
      setIsEditing(false); // On ferme la modale après l'import pour voir le résultat
  };
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="print:hidden pb-40">
          ...
          <div className="p-4 space-y-6">
            ...
            {/* VUE NORMALE SANS LE SELECTEUR (Nettoyée) */}
      ...
      {/* MODAL EDITION (Avec l'option d'import intégrée) */}
      {isEditing && (
        <div className="print:hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                ...
                <form onSubmit={handleUpdate} className="space-y-4">
                    ...
                    {/* --- IMPORT ORDINAIRE DEPLACÉ ICI --- */}
                    {isMesse && availableOrdinaries.length > 0 && (
                        <div className="pt-2 mt-2 border-t border-gray-100 dark:border-slate-800">
                            <label className="text-[10px] font-bold text-orange-600 dark:text-orange-400 uppercase block mb-1">⚡ Remplissage Automatique</label>
                            <select 
                                className="w-full text-sm bg-orange-50 dark:bg-slate-800 border border-orange-200 dark:border-slate-700 rounded-lg px-2 py-2 text-slate-800 dark:text-gray-200"
