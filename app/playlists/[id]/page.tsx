...
// ... (Gardez les constantes MESSE_STRUCTURE et SLOT_KEYWORDS inchangées) ...
...
const SLOT_KEYWORDS: Record<string, string[]> = {
  ...
  "Acclamation": ["alléluia", "alleluia", "acclamation"], "Credo": ["credo", "je crois"],
  "Sanctus": ["sanctus", "saint le seigneur"], "Anamnèse": ["anamnèse", "anamnese"],
  "Agnus Dei": ["agnus", "agneau"], "Entrée": ["entrée", "entree"],
  "Offertoire": ["offertoire"], "Communion": ["communion"], "Envoi": ["envoi", "sortie"],
};

export default function PlaylistDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  
  // ... (Gardez tous les états inchangés : playlist, songs, etc.) ...
  ...
  // ... (Gardez useEffect de chargement, useEffect d'index, handleImportOrdinary inchangés) ...
  useEffect(() => {
  ...
  // ... (Gardez renderPrintContent, renderBlockWithChords, handleShare, triggerPrint, openSearch, formatDate, getAssignedTeamMembers inchangés) ...
  ...
  const renderBlockWithChords = (text: string) => {
      ...
      return parts.map((part, i) => {
          if (part.startsWith('[') && part.endsWith(']')) {
              ...
              return (<span key={i} className="relative inline-block w-0 overflow-visible align-top"><span className="absolute -top-5 left-0 text-red-600 font-bold text-[0.8em] whitespace-nowrap sans-serif">{chordName}</span></span>);
          }
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      <style jsx global>{`@media print { @page { margin: 1cm; size: auto; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } nav, footer, header, .fixed, .sticky, [class*="fixed"], [class*="sticky"], [class*="bottom-0"], [class*="z-50"] { display: none !important; } .print\\:block { display: block !important; position: relative !important; z-index: 9999 !important; } }`}</style>
          ...
          <header className="bg-white dark:bg-slate-900 sticky top-0 z-10 px-4 py-4 border-b border-gray-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden">
                    <div className="flex items-center gap-2"><h1 className="font-bold text-lg text-slate-800 dark:text-white leading-none truncate max-w-[200px]">{playlist.name}</h1></div>
                    <div className="flex gap-2 text-xs mt-1">{playlist.date && <p className="text-orange-600 dark:text-orange-400 font-medium capitalize">{formatDate(playlist.date)}</p>}</div>
                </div>
            </div>
            <div className="flex gap-1 items-center">
                {playlist.externalLink && (<a href={playlist.externalLink} target="_blank" rel="noopener noreferrer" className="p-2 text-blue-600 bg-blue-50 rounded-full hover:bg-blue-100 dark:bg-blue-900/20"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z" /></svg></a>)}
                <button onClick={() => setShowPrintModal(true)} className="p-2 text-slate-600 bg-gray-100 rounded-full hover:bg-gray-200 dark:bg-slate-800 dark:text-gray-300"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg></button>
          ...
          <div className="p-4 space-y-6">
            ...
            {isMesse && availableOrdinaries.length > 0 && (
                <div className="bg-orange-50/50 dark:bg-orange-900/10 p-3 rounded-xl border border-orange-100 dark:border-orange-900/30 flex items-center gap-3">
                    <div className="bg-orange-100 dark:bg-orange-900/30 p-2 rounded-full text-orange-600"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg></div>
                    <div className="flex-1">
                        ...
                        <select className="w-full text-sm bg-white dark:bg-slate-900 border border-orange-200 dark:border-slate-700 rounded-lg px-2 py-1 text-slate-800 dark:text-gray-200" value={selectedOrdinary} onChange={(e) => { setSelectedOrdinary(e.target.value); if(e.target.value) handleImportOrdinary(e.target.value); }}>
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            {stepSongs.map(song => (
                                                <div key={song.id} className="flex items-center justify-between gap-2">
                                                    {/* --- MODIFICATION ICI : Ajout du paramètre playlistId dans l'URL --- */}
                                                    <Link href={`/song/${song.id}?playlistId=${playlist.id}`} className="truncate flex-1 min-w-0">
                        ...
                        songs.map((song, index) => (
                            <div key={song.id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-slate-800 last:border-0 group">
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                    ...
                                    {/* --- MODIFICATION ICI AUSSI : Ajout du paramètre playlistId --- */}
                                    <Link href={`/song/${song.id}?playlistId=${playlist.id}`} className="flex-1 min-w-0">
      ...
      <div className="hidden print:block bg-white text-black w-full">
          <div className="flex items-start gap-4 mb-4 border-b border-gray-300 pb-4">
             ...
             <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600"><span className="font-bold text-orange-600 uppercase">{formatDate(playlist.date)}</span>{playlist.responsable && <span>• Resp: {playlist.responsable}</span>}</div>
                 ...
                 const renderSongItem = (song: any, index: number, label?: string) => (
                    <div key={song.id + index} className="break-inside-avoid w-full">
                        <div className="flex flex-col mb-1 border-l-4 border-slate-800 pl-3">
                            <div className="flex items-baseline gap-2"><h3 className="text-xl font-black uppercase leading-tight">{song.titre}</h3>{label && <span className="text-[10px] font-bold uppercase bg-gray-200 px-1 rounded text-gray-600">{label}</span>}</div>
