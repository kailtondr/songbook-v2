    ...
    <main className="min-h-screen bg-white dark:bg-slate-950 pb-32">
      ...
      <header className="sticky top-0 z-30 bg-white dark:bg-slate-900 pt-3 pb-2 px-3 shadow-sm border-b border-gray-100 dark:border-slate-800">
        ...
        <div className="flex items-center justify-between mb-3">
            ...
            <span className="bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-gray-500 text-[10px] font-bold px-2 py-1 rounded-full">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={() => setShowCategoryModal(true)} className={`flex-1 flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-bold rounded-lg border transition-colors truncate ${selectedCategory ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-gray-100 dark:bg-slate-800 border-transparent text-slate-600 dark:text-gray-300'}`}>
            ...
            <button onClick={() => setShowArtistModal(true)} className={`flex-1 flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-bold rounded-lg border transition-colors truncate ${selectedArtist ? 'bg-orange-50 border-orange-200 text-orange-700' : 'border-gray-200 dark:border-slate-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
      ...
      {showArtistModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in" onClick={() => setShowArtistModal(false)}>
            <div className="bg-white dark:bg-slate-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl h-[70vh] shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b dark:border-slate-800 bg-gray-50 dark:bg-slate-800 flex justify-between items-center"><h3 className="font-bold text-slate-800 dark:text-white">Choisir un Artiste</h3><button onClick={() => setShowArtistModal(false)} className="text-gray-400 text-sm">Fermer</button></div>
                <div className="flex-1 overflow-y-auto p-2">
                    <button onClick={() => { setSelectedArtist(null); setShowArtistModal(false); }} className="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 font-bold text-blue-600 border-b border-gray-100 dark:border-slate-800 mb-2">Tous les artistes</button>
                    {availableArtists.map(artist => (<button key={artist} onClick={() => { setSelectedArtist(artist); setShowArtistModal(false); }} className={`w-full text-left p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 text-sm font-medium ${selectedArtist === artist ? 'text-orange-600 bg-orange-50 font-bold' : 'text-slate-700 dark:text-gray-300'}`}>{artist}</button>))}
                </div>
            </div>
        </div>
      )}
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in" onClick={() => setShowCategoryModal(false)}>
            <div className="bg-white dark:bg-slate-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl h-[60vh] shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b dark:border-slate-800 bg-gray-50 dark:bg-slate-800 flex justify-between items-center"><h3 className="font-bold text-slate-800 dark:text-white">Choisir une Catégorie</h3><button onClick={() => setShowCategoryModal(false)} className="text-gray-400 text-sm">Fermer</button></div>
                <div className="flex-1 overflow-y-auto p-2">
                    <button onClick={() => { setSelectedCategory(null); setShowCategoryModal(false); }} className="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 font-bold text-blue-600 border-b border-gray-100 dark:border-slate-800 mb-2">Toutes les catégories</button>
                    {availableCategories.map(cat => (<button key={cat} onClick={() => { setSelectedCategory(cat); setShowCategoryModal(false); }} className={`w-full text-left p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 text-sm font-medium ${selectedCategory === cat ? 'text-blue-600 bg-blue-50 font-bold' : 'text-slate-700 dark:text-gray-300'}`}>{cat}</button>))}
                </div>
            </div>
        </div>
      )}
      ...
      {showPlaylistModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl flex flex-col max-h-[80vh] border dark:border-slate-800">
                <div className="p-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-800"><h3 className="font-bold text-slate-800 dark:text-white">Ajouter à...</h3><button onClick={() => setShowPlaylistModal(false)} className="text-gray-400 hover:text-white">✕</button></div>
                <div className="p-3 border-b border-gray-100 dark:border-slate-800 bg-orange-50/50 dark:bg-orange-900/10">
                    {!isCreatingPl ? (<button onClick={() => setIsCreatingPl(true)} className="w-full py-2 border-2 border-dashed border-orange-200 dark:border-orange-800 text-orange-600 dark:text-orange-400 font-bold rounded-xl text-sm">+ Nouvelle Playlist</button>) : (<form onSubmit={createAndAddPlaylist} className="flex gap-2"><input autoFocus type="text" placeholder="Nom..." className="flex-1 px-3 py-2 rounded-lg border border-orange-200 dark:border-orange-800 dark:bg-slate-800 dark:text-white text-sm" value={newPlaylistName} onChange={e => setNewPlaylistName(e.target.value)} /><button type="submit" className="bg-orange-600 text-white px-3 py-2 rounded-lg font-bold text-xs">OK</button></form>)}
                </div>
                <div className="overflow-y-auto p-2 flex-1">
                    {playlists.map(pl => (<button key={pl.id} onClick={() => addToPlaylist(pl.id, pl.name)} className="w-full text-left p-3 hover:bg-orange-50 dark:hover:bg-slate-800 rounded-xl flex items-center justify-between group border-b border-gray-50 dark:border-slate-800 last:border-0"><span className="font-bold text-slate-700 dark:text-slate-300">{pl.name}</span><span className="text-xs bg-gray-100 dark:bg-slate-800 text-gray-500 px-2 py-1 rounded-full">{pl.songs?.length || 0}</span></button>))}
                </div>
            </div>
        </div>
      )}
